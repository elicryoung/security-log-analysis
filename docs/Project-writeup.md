...
## Section 3: IIS Webshell and Sysmon Process Analysis

This dataset was probably the first one where I genuinely felt like I was investigating attacker behaviour instead of just reading logs. By this point in the project, I had started becoming much more comfortable with both the structure of Windows events and using Linux command-line tools to investigate them.

The dataset analysed was:

3.LM_typical_IIS_webshell_sysmon_1_10_traces.evtx

After converting the EVTX file into XML format, I immediately noticed the logs looked different from the previous datasets because these events were generated by Sysmon rather than standard Windows Security logs.

One of the biggest differences with Sysmon logs is that they contain detailed process telemetry, including:
- parent processes
- command lines
...
At the start of this project, I would have probably just scrolled through the XML manually and hoped something suspicious stood out. By this stage though, my thought process had started changing. Instead of trying to read everything, I started thinking more like:
- What processes are running?
- Which processes spawned other processes?
- What command lines were executed?
- Are multiple events connected by the same Logon ID?
- Does the process chain make sense for normal behaviour?

The first thing I checked was the Event Record IDs using:

bash id="jfe9k9" grep -i "EventRecordID" webshell.xml | sort | uniq -c 

which returned:

text id="8kh8bn" 1 <EventRecordID>1044</EventRecordID> 1 <EventRecordID>1045</EventRecordID> 1 <EventRecordID>1046</EventRecordID> 1 <EventRecordID>1047</EventRecordID> 

This helped confirm that the dataset was showing a very small and connected sequence of events rather than unrelated activity.

I then started investigating process IDs using:

bash id="r1s7n7" grep -i "ProcessID" webshell.xml | sort | uniq -c 

At this point I was trying to understand how the different processes connected together.

One thing that stood out immediately was the repeated use of:
- cmd.exe
- net.exe
- net1.exe
...
I then checked the command lines using:

bash id="8mw2rj" grep -i "CommandLine" webshell.xml | sort | uniq -c 

which returned:

text id="4lljpv" <Data Name="CommandLine">"c:\windows\system32\cmd.exe" /c net user</Data> <Data Name="CommandLine">C:\Windows\system32\net1  user</Data> <Data Name="CommandLine">net  user</Data> 

This was probably the moment where the overall picture started becoming clear.

The command:

cmd id="2v90mo" cmd.exe /c net user 

showed that a command shell was executing net user, which is commonly used to enumerate Windows accounts.

The important part though was not just the command itself, but which process launched it.

I investigated the parent processes using:

bash id="j4p6z2" grep -i "ParentImage" webshell.xml | sort | uniq -c 

which returned:

text id="xgrr7k" <Data Name="ParentImage">C:\Windows\System32\cmd.exe</Data> <Data Name="ParentImage">C:\Windows\System32\inetsrv\w3wp.exe</Data> <Data Name="ParentImage">C:\Windows\System32\net.exe</Data> 

This allowed me to reconstruct the process chain:

text id="mh4pfc" w3wp.exe  └── cmd.exe       └── net.exe            └── net1.exe 

This was one of the biggest learning moments in the project so far because it was the first time I properly understood parent-child process relationships inside Windows telemetry.

The reason this stood out as suspicious is because:
- w3wp.exe is the IIS web server process
- web servers normally should not be spawning command shells
- the shell was then executing account enumeration commands

This strongly suggested possible:
- webshell activity
- remote command execution
- attacker reconnaissance behaviour

I also investigated Logon IDs using:

bash id="ojtv98" grep -i "LogonId" webshell.xml | sort | uniq -c 

which returned:

text id="rj9t1t" 3 <Data Name="LogonId">0x00000000009cf992</Data> 

By this stage of the project, I already understood that matching Logon IDs often indicate related activity within the same authenticated session. Seeing the same Logon ID repeated across multiple processes helped confirm that the events were all connected to the same execution flow.

One thing I noticed during this investigation was that I had started naturally thinking about which Linux commands would help answer specific questions rather than randomly searching the logs. Earlier in the project I mostly used grep just to search for words, but during this investigation I started using commands with an actual purpose:
- searching for process relationships
- identifying repeated IDs
- correlating sessions
- reconstructing execution chains
- narrowing down suspicious activity

I also made mistakes during the investigation that helped me understand Linux behaviour better. For example, I initially ran:

bash id="byobfk" grep <EventRecordID> webshell.xml 

which caused a shell error because < > are treated as redirection operators in bash. After correcting the syntax using quotation marks, I understood why quoting search strings matters in Linux commands.

Another thing I started understanding during this section was the difference between useful and noisy searches. For example:

bash id="gljlwm" grep -i "User" webshell.xml 

returned both actual user fields and command lines containing the word user, which made the output noisy. This helped me realise that more specific searches produce much cleaner investigative results.

Compared to the earlier Windows datasets, this section felt much more like a real SOC investigation because I was no longer just identifying Event IDs — I was actually reconstructing attacker behaviour using process telemetry and command-line evidence.

By the end of this investigation, I had a much better understanding of:
...
- using Linux commands to investigate Windows process activity

This section was probably the biggest step forward in the project so far because it was the first time I felt like I could follow the actual flow of attacker activity through the logs instead of just reading isolated events.